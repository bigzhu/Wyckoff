# 第四节 冰线交易法与 SOT 停止行为

## 核心思想

本节探讨如何精准定位趋势转折点，涵盖了熊市背景下的“**[冰线](../术语速查手册.md#ice-line-ice)交易法**”——即利用恐慌抛售后的每一次反弹来识别底部的构建，以及上涨中利用“**[SOT](../术语速查手册.md#sot-shortening-of-thrust) (Shortening of Thrust)**”识别动力衰竭的技巧。

---

## 一、案例 7：冰线交易法 (图 5-9)

### 1. 案例分析 (熊市中的反弹与突破)

![图 5-9：熊市通道与冰线测试](../wyckoff_content/OEBPS/Image00109.jpg)

- **趋势背景**：大环境为明显的熊市通道。
- **突破逻辑 (3)**：蜡烛 3 强力突破**[冰线](../术语速查手册.md#ice-line-ice)**和供应线。虽然表现强劲，但因速度过快，预判将有超买回落。
- **支撑定位**：
    - **第一支撑**：**[冰线](../术语速查手册.md#ice-line-ice)**（原阻力转支撑）。
    - **第二支撑**：蜡烛 3 的最低点。
    - **多空分界**：回调若超过 1-3 涨幅的 50%，则看涨逻辑失效。

#### 冰线突破策略逻辑

```mermaid
graph TD
    A[熊市背景] --> B[价格突破**[冰线](../术语速查手册.md#ice-line-ice)**/供应线]
    B --> C{突破力度?}
    C -- 过强 (超买) --> D[必然回落测试]
    D --> E{回落深度?}
    E -- >50% --> F[失败: 熊市延续]
    E -- <50% + **[Spring](../术语速查手册.md#spring)** --> G[成功: 最佳进场点]
```

### 2. [实战结果] 验证分析 (图 6-10)

![图 6-10：支撑位吸收与 Spring](../wyckoff_content/OEBPS/Image00141.jpg)

- **调整验证**：价格回落并持稳于蜡烛 3 低点之上，说明卖盘被吸收。
- **进场点**：在 50% 位置出现的 **[Spring](../术语速查手册.md#spring)** 是绝佳的补仓/进场机会。

---

## 二、案例 8：SOT 停止行为与二次测试 (图 5-10)

### 1. 案例分析 (上涨幅度的衰减)

![图 5-10：SOT 识别与测试逻辑](../wyckoff_content/OEBPS/Image00110.jpg)

- **停止信号 ([SOT](../术语速查手册.md#sot-shortening-of-thrust))**：蜡烛 1 虽然突破了供应线，但随后阳线长度缩短，显示上涨动力（Thrust）在递减。
- **虚假破位 (2)**：蜡烛 2 虽带量创新低，但收盘在中点之上，实际上是 [CM(狗庄)](../术语速查手册.md#cm-composite-man) 在吸收割肉仓位的震仓。
- **关键测试 (3)**：随后的成交量大幅萎缩，说明供应已彻底枯竭，行情看涨。

#### SOT 动力衰竭模型

```mermaid
graph TD
    A[价格连续创新高] --> B[观察波段幅度 (Thrust)]
    B --> C{幅度表现?}
    C -- 递增 --> D[趋势强劲]
    C -- 递减 (SOT) --> E[动力衰竭: 停止行为]
    E --> F[等待二次测试确认]
    F -- 缩量 --> G[反转确认]
```

### 2. [实战结果] 验证分析 (图 6-11)

![图 6-11：JOC 后的趋势确认](../wyckoff_content/OEBPS/Image00142.jpg)

- **趋势管控**：阳线低点不断抬高，确认需求控制市场。
- **转弱预警 (5-7)**：蜡烛 5 处的回测成交量过大且反弹无力，随后的 6-7 出现放量阴线，说明二次测试失败，行情转跌。

---

## 三、案例 17：阻力位后的“跟随”关键性 (图 5-22)

### 1. 案例分析 (拒绝下跌逻辑)

![图 5-22：阻力位的 SOT 与破位尝试](../wyckoff_content/OEBPS/Image00122.jpg)

- **停止信号 (1)**：阻力位出现 SOT。
- **跟随博弈 (2)**：蜡烛 2 跌破支撑线，直观上利空。但成交量未显著扩大，是否是“陷阱”取决于次日的跟随表现。

### 2. [实战结果] 验证分析 (图 6-21)

![图 6-21：吸收卖盘后上涨](../wyckoff_content/OEBPS/Image00152.jpg)

- **吸收证据 (3-4)**：蜡烛 3 迅速反抽拒绝下跌。虽然 4 处的二次测试成交量仍大，但阳线收盘证明需求在积极吃货。
- **最终确认**：低位下跌幅度小于上涨，供应耗尽，最终迎来 **[JOC](../术语速查手册.md#joc-jump-across-the-creek) 突破**。

---

## 📈 核心知识总结

| 信号/工具                                                                  | 实战意义       | 核心逻辑                                                       |
| :------------------------------------------------------------------------- | :------------- | :------------------------------------------------------------- |
| **[冰线突破](../术语速查手册.md#ice-line-ice)**                            | 熊市结束的信号 | 突破后回测不深且量缩，是反转确认。                             |
| **[SOT (Shorten of Thrust)](../术语速查手册.md#sot-shortening-of-thrust)** | 预判趋势回调   | 连续突破的幅度越来越小，说明动力在流失。                       |
| **跟随 (Follow-through)**                                                  | 判定胜负手     | 每一个威胁信号（如破位阴线）都必须看到连续的卖盘跟随才算真实。 |
| **50% 原则**                                                               | 衡量趋势强度   | 回调不破前波 50%，趋势依然健康。                               |

---

## 🔗 导航

- [← 第三节: 支撑阻力与吸收行为分析](./第三节_支撑阻力与吸收行为分析.md)
- [→ 第五节: 震荡区分析与震仓实战](./第五节_震荡区分析与震仓实战.md)
- [↑ 返回总目录](../README.md)
